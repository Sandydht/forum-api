...
describe('DomainErrorTranslator', () => {
  it('should translate error correctly', () => {
    ...
    expect(DomainErrorTranslator.translate(new Error('USER_LOGIN.NOT_MEET_DATA_TYPE_SPESIFICATION'))).toStrictEqual(new InvariantError('username dan password harus string'));

    expect(DomainErrorTranslator.translate(new Error('LOGOUT_USER_USE_CASE.NOT_CONTAIN_NEEDED_PROPERTY'))).toStrictEqual(new InvariantError('harus mengirimkan token refresh'));
    expect(DomainErrorTranslator.translate(new Error('LOGOUT_USER_USE_CASE.PAYLOAD_NOT_MEET_DATA_TYPE_SPESIFICATION'))).toStrictEqual(new InvariantError('refresh token harus string'));

    expect(DomainErrorTranslator.translate(new Error('REFRESH_AUTHENTICATION_USE_CASE.NOT_CONTAIN_REFRESH_TOKEN'))).toStrictEqual(new InvariantError('harus mengirimkan token refresh token'));
    expect(DomainErrorTranslator.translate(new Error('REFRESH_AUTHENTICATION_USE_CASE.PAYLOAD_NOT_MEET_DATA_TYPE_SPESIFICATION'))).toStrictEqual(new InvariantError('refresh token harus string'));
  });
